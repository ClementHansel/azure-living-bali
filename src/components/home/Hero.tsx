...
export default function Hero() {
  ...
  useEffect(() => {
    const handleWheel = (e: WheelEvent) => {
      ...
      if (e.deltaY > 10)
        setActiveIndex((prev) => Math.min(prev + 1, MENU.length - 1));
      else if (e.deltaY < -10) setActiveIndex((prev) => Math.max(prev - 1, 0));
    };
  ...
  return (
    ...
    >
      ...
      <div className="sticky top-0 h-[100svh] w-full">
        ...
        {/* Right menu */}
        <div className="absolute bottom-35 right-6 md:bottom-25 md:right-16 z-30 text-right">
        ...
        {/* WhatsApp CTA as a separate section */}
        <div className="relative flex justify-end px-6 md:px-16 mt-6 z-30">
          <a
            href="https://wa.me/4917682360647"
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-3 bg-[#25D366] text-white px-5 py-3 rounded-full shadow-lg hover:shadow-2xl transition-all hover:scale-105 active:scale-95"
